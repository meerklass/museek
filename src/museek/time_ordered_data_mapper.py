...
class TimeOrderedDataMapper:
    ...
    def __init__(
        ...
        if flags is not None:
            ...
            self._channel_iterator = DataElement.flagged_channel_iterator(
                data_element=self._to_map, flag_element=self._flags
            )
        else:
            self._flags = None
            self._channel_iterator = DataElement.channel_iterator(
                data_element=self._to_map
            )

    @classmethod
    def from_time_ordered_data(
        cls, data: TimeOrderedData, recv: int = 0, flag_threshold: int = 1
        ...
        """
        return cls(
            right_ascension=data.right_ascension.get(recv=recv),
    ...
    def grid(
        self, grid_size: tuple[int, int] = (60, 60), method: str = "linear"
        ...
        """
        right_ascension_i = np.linspace(
            self._right_ascension.squeeze.min(),
            self._right_ascension.squeeze.max(),
            grid_size[0],
        )
        declination_i = np.linspace(
            self._declination.squeeze.min(),
            self._declination.squeeze.max(),
            grid_size[1],
        )
        ...
        maps = [
            (
                griddata(
                    (
                        self._right_ascension.get(time=unmasked).squeeze,
                        self._declination.get(time=unmasked).squeeze,
                    ),
            ...
            for channel, unmasked in self._channel_iterator
        ]
        if self._flags:
            masks = [
                griddata(
