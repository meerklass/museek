...
class SynchrotronTemperature:
    ...
    def __init__(
        ...
        self._coarse_freq_MHz = np.arange(
            freq_MHz_min, freq_MHz_max + freq_step_MHz, freq_step_MHz
        )
        n_coarse = len(self._coarse_freq_MHz)
        n_pix = hp.nside2npix(nside)

        print(
            f"Computing synchrotron model on {n_coarse} coarse frequencies "
            f"({freq_MHz_min:.0f}–{freq_MHz_max:.0f} MHz, step {freq_step_MHz:.0f} MHz)...",
            flush=True,
        )
        ...
        for i, f_MHz in enumerate(self._coarse_freq_MHz):
            ...
            self._smoothed_maps[i] = pysm3.apply_smoothing_and_coord_transform(
                emission_map, fwhm=fwhm
            )

        print("Synchrotron model ready.", flush=True)

    def get_temperature(self, ra: np.ndarray, dec: np.ndarray) -> np.ndarray:
        """
        ...
        for i_dump in range(n_dumps):
            synch_temp[i_dump] = np.interp(
                self._freq_MHz, self._coarse_freq_MHz, synch_coarse[i_dump]
            )
