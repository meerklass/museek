...
def calculate_median_coordinates_excluding_flagged_antennas(
    ...
    for idx in flag_indices:
        combined_flag_array = np.logical_or(
            combined_flag_array, track_data.flags._flags[idx].get_array()
        )
    ...
    receiver_indices = np.arange(
        0, combined_flag_array.shape[2], 2
    )  # [0, 2, 4, 6, ...]
...
class PointSourceCalibrationPlugin(AbstractParallelJoblibPlugin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.gain_threshold_scales = (
            gain_threshold_scales if gain_threshold_scales is not None else [1.0]
        )
        ...
        if gain_method not in ("gain_corr", "gain_on_off"):
            raise ValueError(
                f"gain_method must be 'gain_corr' or 'gain_on_off', got {gain_method!r}."
            )
    ...
    def set_requirements(self):
        ...
        self.requirements = [
            ...
            Requirement(
                location=ResultEnum.CALIBRATOR_NAMES, variable="calibrator_names"
            ),
    ...
    def map(
        ...
        for i_receiver, receiver in enumerate(track_data.receivers):
            ...
            atm_emission_data[i_receiver] = atmospheric_model.emission_temperature[
                :, :, antenna_idx
            ]
        ...
        beam_solid_angle_HH = beam_model.get_beam_solid_angle_at_freq(
            frequency_MHz=freq_MHz, polarization="HH"
        )  # Shape: (n_freq,)
        beam_solid_angle_VV = beam_model.get_beam_solid_angle_at_freq(
            frequency_MHz=freq_MHz, polarization="VV"
        )  # Shape: (n_freq,)
        ...
        for period in calibrator_validated_periods:
            ...
            point_source_flux, ra_deg, dec_deg = get_point_source_model(
                calibrator_name, freq
            )
            ...
            median_pointing = SkyCoord(
                az=az_median_period * u.deg,
                alt=el_median_period * u.deg,
                frame=altaz_frame_period,
            )
            ...
            spillover_temp_HH = spillover_model.get_temperature(
                elevation=el_median_period, frequency_MHz=freq_MHz, polarization="HH"
            )  # Shape: (n_dumps_period, n_freq)

            spillover_temp_VV = spillover_model.get_temperature(
                elevation=el_median_period, frequency_MHz=freq_MHz, polarization="VV"
            )  # Shape: (n_dumps_period, n_freq)

            # Calculate point source temperature for both polarizations
            # T = (λ² × 10^-26 / (2 k_B)) × S_ν[Jy] × G_beam / Ω_beam
            conversion_factor = (wavelength_m**2 * 1e-26) / (
                2 * k_B
            )  # [K·sr/Jy], shape (n_freq,)
            ...
            on_mask = (
                source_altaz.separation(median_pointing).deg
                < self.on_source_separation_threshold_deg
            )

            # Count on-source pointings = contiguous on-source dump groups (rising edges of on_mask).
            n_on_found = int(
                (np.diff(np.concatenate(([0], on_mask.astype(int), [0]))) == 1).sum()
            )
            print(
                f"Period {period}: {on_mask.sum()} on-source dumps in {n_on_found} pointing(s), "
                f"{(~on_mask).sum()} off-source dumps",
                flush=True,
            )
            ...
            for i_receiver, receiver in enumerate(track_data.receivers):
                ...
                spillover_temp_period = (
                    spillover_temp_HH if polarization == "HH" else spillover_temp_VV
                )
                point_source_temp_period = (
                    point_source_temp_HH
                    if polarization == "HH"
                    else point_source_temp_VV
                )
                ...
                rec_temp_recv = rec_temp_data[
                    i_receiver
                ]  # Shape: (n_freq,), constant in time

                # Slice data to this specific period
                vis_period = visibility_recv.squeeze[select]  # (n_dumps_period, n_freq)
                flag_period = initial_flag_recv.squeeze[
                    select
                ]  # (n_dumps_period, n_freq)
                atm_period = atm_emission_recv[select, :]  # (n_dumps_period, n_freq)

                # Store receiver-specific data only
                self.model_components[period]["temperatures"]["atmospheric"].append(
                    atm_period
                )
                self.model_components[period]["temperatures"]["receiver"].append(
                    rec_temp_recv
                )
    ...
    def run_job(self, anything: tuple) -> dict:
        ...
        if self.verbose:
            print(
                f"Period {period}, Calibrator {calibrator_name}: "
        ...
        def _flag_gain_1d(gain: np.ma.MaskedArray) -> np.ma.MaskedArray:
            ...
            rfi_mask = get_rfi_mask_1d(
                time_ordered=DataElement(
                    array=np.ma.filled(gain, 0.0)[:, np.newaxis, np.newaxis]
                ),
        ...
        if self.gain_method == "gain_corr":
            ...
            if on_mask.any() and off_mask.any():
                ...
                if self.verbose:
                    print(
                        f"Period {period}: skipping gain_on_off (no on or off dumps)",
                        flush=True,
                    )
        ...
        if self.verbose:
            print(
                f"Period {period}: gain ({self.gain_method}) at 750 MHz = "
                f"{selected_gain[idx_750]:.6f}",
                flush=True,
            )
    ...
    def gather_and_set_result(
        ...
        for period in period_keys_stack:
            ...
            self.model_components[period]["temperatures"]["atmospheric"] = np.stack(
                self.model_components[period]["temperatures"]["atmospheric"], axis=2
            )
            # Stack receiver: list of (n_freq,) -> (n_freq, n_receivers)
            self.model_components[period]["temperatures"]["receiver"] = np.stack(
                self.model_components[period]["temperatures"]["receiver"], axis=1
            )
        ...
        calibrators_processed = set(
            [
                self.model_components[period]["calibrator"]
                for period in period_keys_stack
            ]
        )
        ...
        if self.verbose:
            self._plot_coordinate_comparison(
                track_data=track_data, output_path=output_path
            )
        ...
        self.set_result(
            result=Result(
                location=ResultEnum.MODEL_COMPONENTS,
    ...
    def _plot_coordinate_comparison(
        self, track_data: TimeOrderedData, output_path: str
    ):
        """
        ...
        ax1.plot(
            ...
            color="blue",
            linewidth=2,
            label="Median (excluding flagged)",
            linestyle="--",
        )
        ax1.plot(
            time_indices,
        ...
        ax1.set_title(
            f"Azimuth: Median (excluding flagged antennas) vs Receiver {antenna_name}"
        )
        ax1.legend()
        ...
        ax2.set_title(
            f"Elevation: Median (excluding flagged antennas) vs Receiver {antenna_name}"
        )
        ax2.legend()
        ...
        plot_filename = os.path.join(
            output_path, f"coordinate_comparison_median_vs_{antenna_name}.png"
        )
