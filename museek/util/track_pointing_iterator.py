...
class TrackPointingIterator:
    ...
    def __init__(
        ...
        pointing_slewing_thresholds: tuple[float, float] = (7.0, 15.0),
        min_max_pointing_time: tuple[float, float] = (55.0, 80.0),
    ):
        """
        Initialise
        ...
        if len(calibrator_observation_labels) != n_calibrator_observations:
            raise ValueError(
                f"The length of `calibrator_observation_labels` must match `n_calibrator_observations`,"
    ...
    def _target_dumps_one_calibrator(self) -> list[range | None]:
        ...
        calibrator_index = np.argmin(
            np.abs(np.asarray([self._scan_start, self._scan_end]) - self._features[0])
        )
    ...
    def _two_calibrator_observations(self) -> bool:
        ...
        if (
            max_diff < 0.6 * self._scan_observation_duration
        ):  # at least 1 hour gap for scanning observation
            return False
        return True
    ...
    def _single_dish_calibrators(
        ...
        for dumps, label in zip(target_dumps_list, self._calibrator_observation_labels):
            ...
            pointing_change_indices = np.where(
                (features_diff > lower) & (features_diff < upper)
            )[0]
            pointing_change_dumps = np.asarray(
                [features[i] for i in pointing_change_indices]
            )
            pointing_change_dumps_diff = (
                pointing_change_dumps[1:] - pointing_change_dumps[:-1]
            )

            if not all(
                (pointing_change_dumps_diff > min_pointing_time)
            ...
            if (
                ...
                and max(features_diff[range(*valid_indices)]) > upper
            ):
            ...
            if self._plot_dir is not None:
                ...
                plot_name = (
                    f"track_pointing_iterator_single_dish_calibrators_{label}.png"
                )
                plt.xlabel("time [s]")
            ...
            if (
                len(pointing_change_indices) != n_calibrator_pointings - 1
                or valid_indices is None
            ):
