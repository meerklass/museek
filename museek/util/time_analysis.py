...
class TimeAnalysis:
    ...
    def time_difference_to_sunset_sunrise(
        ...
        if (
            abs(
                obs_start - observer.previous_setting(ephem.Sun()).datetime()
            ).total_seconds()
            > abs(
                obs_start - observer.next_setting(ephem.Sun()).datetime()
            ).total_seconds()
        ):
        ...
        if (
            abs(
                obs_end - observer.previous_rising(ephem.Sun()).datetime()
            ).total_seconds()
            > abs(
                obs_end - observer.next_rising(ephem.Sun()).datetime()
            ).total_seconds()
        ):
