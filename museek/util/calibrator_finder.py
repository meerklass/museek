...
def _find_calibrator_scans_in_period(
    ...
    for scan_tuple in track_data._scan_tuple_list:
        if scan_tuple.state == ScanStateEnum.TRACK:
            ...
            if (
                in_period
                and _matches_calibrator_name(scan_tuple.target.name, calibrator_name)
                and scan_duration > min_duration_seconds
            ):
                matching_scans.append(
                    (
                        scan_tuple.target.name,
                        trimmed_dumps,
                        scan_start_time,
                        scan_end_time,
                    )
