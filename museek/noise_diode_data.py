...
class NoiseDiodeData(TimeOrderedData):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.noise_diode = NoiseDiode(
            dump_period=self.dump_period, observation_log=self.obs_script_log
        )
        if (
            self.scan_state == ScanStateEnum.SCAN
            or self.scan_state == ScanStateEnum.TRACK
        ):
    ...
    def _dumps(self) -> list[int]:
        ...
        if (
            self.scan_state not in [ScanStateEnum.SCAN, ScanStateEnum.TRACK]
            or self.noise_diode is None
        ):
            return dumps_of_scan_state
        return [
            i
            for i in dumps_of_scan_state
            if i
            in self.noise_diode.get_noise_diode_off_scan_dumps(
                timestamps=self.original_timestamps
            )
        ]
