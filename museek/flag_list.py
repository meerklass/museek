...
class FlagList:
    ...
    def __init__(
        ...
        self._flag_descriptions: list[str] = (
            descriptions if descriptions is not None else [""] * len(flags)
        )
        self._check_flags()
    ...
    def from_array(
        ...
        return cls(
            flags=[element_factory.create(array=flag) for flag in array],
            names=names,
    ...
    def flag_names(self) -> list[str]:
        ...
        # Fallback for `FlagList`s unpickled from contexts created before names existed.
        if not hasattr(self, "_flag_names"):
            self._flag_names = [""] * len(self._flags)
        return self._flag_names

    @property
    def flag_descriptions(self) -> list[str]:
        """Return the list of flag descriptions, one per flag."""
        # Fallback for `FlagList`s unpickled from contexts created before descriptions existed.
        if not hasattr(self, "_flag_descriptions"):
            self._flag_descriptions = [""] * len(self._flags)
        return self._flag_descriptions

    def add_flag(
        self, flag: Union[FlagElement, "FlagList"], name: str = "", description: str = ""
    ):
        """
        ...
        # Reference the guarded properties so `FlagList`s unpickled from older
        # contexts (without these attributes) are initialised before mutation.
        self.flag_names.append(name)
        self.flag_descriptions.append(description)
        self._check_flags()
    ...
    def remove_flag(self, index: int):
        ...
        self._flag_names = [
            name for i, name in enumerate(self.flag_names) if i != index
        ]
        self._flag_descriptions = [
            desc for i, desc in enumerate(self.flag_descriptions) if i != index
        ]
        self._check_flags()
    ...
    def get(self, **kwargs) -> "FlagList":
        ...
        return FlagList(
            flags=[flag.get(**kwargs) for flag in self._flags],
            names=list(self.flag_names),
            descriptions=list(self.flag_descriptions),
        )
