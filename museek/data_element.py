...
class DataElement(AbstractDataElement):
    ...
    def __mul__(
        self, other: Union["DataElement", np.ndarray, numbers.Number]
    ) -> "DataElement":
        """
        ...
        if not isinstance(other, numbers.Number):
            if self.shape != other.shape:
                raise ValueError(
                    f"Cannot multiply instances with different shapes, "
    ...
    def __sub__(
        ...
        if not isinstance(other, numbers.Number):
            if self.shape != other.shape:
                raise ValueError(
                    f"Cannot subtract instances with different shapes, "
                    f"got {self.shape} and {other.shape}."
                )
        if isinstance(other, DataElement):
            return DataElement(array=self.array - other.array)
    ...
    def __add__(
        ...
        if not isinstance(other, numbers.Number):
            if self.shape != other.shape:
                raise ValueError(
                    f"Cannot add instances with different shapes, "
                    f"got {self.shape} and {other.shape}."
                )
        if isinstance(other, DataElement):
            return DataElement(array=self.array + other.array)
    ...
    def _kurtosis(self, axis: int | list[int, int] | tuple[int, int]) -> "DataElement":
        ...
        return DataElement(
            array=scipy.stats.kurtosis(self.array, axis=axis, keepdims=True)
        )
