        ...
        for receiver in track_data.receivers:
            ...
            for (
                ...
                plot_name = os.path.join(
                    receiver_path,
                    f"single_dish_calibrator_plugin_{before_or_after}.png",
                )
                print(f"Saving plot {plot_name}.")
                colors = ["red", "blue", "green", "orange", "yellow", "black", "purple"]
                for centre, pointing_times, color in zip(
                    pointing_centres, pointing_times_list, colors
                ):
                    track_times = list(np.asarray(times)[pointing_times])
                    plt.scatter(centre[0], centre[1], marker="x", s=100, c=color)
                    right_ascension = track_data.right_ascension.get(
                        time=track_times, recv=i_antenna
                    ).squeeze
                    declination = track_data.declination.get(
                        time=track_times, recv=i_antenna
                    ).squeeze
