...
class ReadCalibratorGainsPlugin(AbstractPlugin):
    ...
    def run(self, scan_data: TimeOrderedData, output_path: str):
        ...
        if len(files) > 2:
            raise ValueError(
                f"At most two model_components files are supported, got {len(files)}."
            )
        ...
        for path in files:
            ...
            if "receivers" not in mc:
                raise ValueError(
                    f"Pickle file {path} does not contain receiver labels. "
        ...
        if self.periods is None:
            if len(files) == 2:
                raise ValueError(
                    "Two model_components files were given; specify `periods` as one "
            ...
            if len(self.periods) != 2:
                raise ValueError(
                    f"Two files were given, so `periods` must have exactly two period "
                    f"names (one per file); got {self.periods}."
                )
            chosen = [(0, self.periods[0]), (1, self.periods[1])]
        else:
            chosen = [
                (0, p) for p in self.periods
            ]  # one file -> all listed periods come from it
        ...
        for i_file, period in chosen:
            ...
            if period not in mc:
                raise ValueError(
                    f"Period '{period}' not found in {path}. "
                    f"Available periods: {available}."
                )
            gain_key = (
                "gain" if "gain" in mc[period] else "gain_on_off"
        ...
        if not selections:
            raise ValueError(
                "No calibrator periods selected from the provided pickle files."
            )
        ...
        mask_matched = np.ones(
            (n_freq, len(current_receivers)), dtype=bool
        ...
        for i, recv in enumerate(current_receivers):
            ...
            for gain_arr, file_receivers, _label in selections:
                if recv in file_receivers:
                    recv_gains.append(
                        gain_arr[:, file_receivers.index(recv)]
                    )  # (n_freq,) masked

            if len(recv_gains) == 0:
                print(
                    f"Warning: receiver {recv} not found in any selected gain — masking.",
                    flush=True,
                )
            ...
            if len(recv_gains) < len(selections):
                print(
                    f"Warning: receiver {recv} found in only {len(recv_gains)}/{len(selections)} "
                    f"selections — using partial average.",
                    flush=True,
                )
        ...
        scan_data.flags.add_flag(
            flag=FlagElement(array=gain_mask), name="gain_solution_mask"
        )
        self.set_result(
            result=Result(
        ...
        self.set_result(
            result=Result(location=ResultEnum.CALIBRATOR_GAIN, result=gain_matched)
        )

        if self.verbose:
            print(
                f"Loaded selections: {[label for _, _, label in selections]}",
                flush=True,
            )
            print(f"Calibrator gain shape: {gain_matched.shape}", flush=True)
            unmasked = np.where(~mask_matched[:, 0])[0]
            freq_mid = (
                unmasked[len(unmasked) // 2] if len(unmasked) > 0 else n_freq // 2
            )
            for i, recv in enumerate(current_receivers):
                print(
                    f"  {recv}: gain[mid_freq] = {gain_matched[freq_mid, i]:.4f}",
                    flush=True,
                )
