...
class AntennaFlaggerPlugin(AbstractPlugin):
    ...
    def run(
        ...
        self.set_result(
            result=Result(location=ResultEnum.TRACK_DATA, result=track_data)
        )
        self.set_result(
            result=Result(
    ...
    def flag_outlier_antennas(self, data: TimeOrderedData):
        ...
        _, antennas = self.outlier_antenna_indices(
            data=data, distance_threshold=self.outlier_threshold
        )
    ...
    def flag_outlier_antennas_TOD(self, data: TimeOrderedData):
        ...
        for i_timestamps, timestamps in enumerate(data.timestamps):
            ...
            feature = np.asarray([antenna_elevation, antenna_azimuth]).T
            # Ensure feature is always 2D for clustering
            if feature.ndim == 1:
                feature = feature.reshape(1, -1)
            outlier_indices = Clustering().iterative_outlier_indices(
                feature_vector=feature, distance_threshold=self.outlier_threshold
            )

            new_flag_array[i_timestamps, outlier_indices] = 1.0
    ...
    def outlier_antenna_indices(
        data: TimeOrderedData, distance_threshold: float
    ) -> tuple[list[int], list[Antenna]]:
        """
        ...
        antenna_azimuth_end = data.azimuth.get(
            time=data.timestamps.shape[0] - 1
        ).squeeze
        antenna_azimuth_middle = data.azimuth.get(
            time=data.timestamps.shape[0] // 2
        ).squeeze

        feature = np.asarray(
            [
                antenna_elevation_mean,
                antenna_azimuth_min,
                antenna_azimuth_max,
                antenna_azimuth_start,
                antenna_azimuth_end,
                antenna_azimuth_middle,
            ]
        ).T
        outlier_indices = Clustering().iterative_outlier_indices(
            feature_vector=feature, distance_threshold=distance_threshold
        )
    ...
    def flag_for_elevation(self, data: TimeOrderedData):
        ...
        for antenna in ConstantElevationScans.get_antennas_with_non_constant_elevation(
            data=data, threshold=self.elevation_std_threshold
        ):
    ...
    def flag_for_elevation_TOD(self, data: TimeOrderedData):
        ...
        for i_antenna, antenna in enumerate(data.antennas):
            ...
            bad_elevation_flag = (
                abs(antenna_elevation - np.median(antenna_elevation))
                > self.elevation_threshold
            )

            ##### dilate the flag #####
            struct = np.ones(5, dtype=bool)
            bad_elevation_flag = ndimage.binary_dilation(
                bad_elevation_flag, structure=struct, iterations=2
            )
            ...
            for i_receiver in i_receiver_list:
                if standard_deviation > self.elevation_std_threshold:
                    ...
                    flag_array = np.repeat(bad_elevation_flag, shape[1]).reshape(
                        (shape[0], shape[1], 1)
                    )
                    new_flag.insert_receiver_flag(
                        flag=DataElement(array=flag_array),
