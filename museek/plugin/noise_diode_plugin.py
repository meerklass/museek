...
class NoiseDiodePlugin(AbstractParallelJoblibPlugin):
    ...
    def map(
        ...
        noise_diode_off_dumps = noise_diode.get_noise_diode_off_scan_dumps(
            timestamps=scan_data.timestamps
        )
    ...
    def run_job(
        self,
        anything: tuple[DataElement, np.ndarray, np.ndarray, np.ndarray, np.ndarray],
    ...
    def gather_and_set_result(
        ...
        receiver_mask_poly_fit = remove_outliers_zscore_mad(
            rmsnorm_poly_fit_list,
            np.isnan(rmsnorm_poly_fit_list),
        ...
        for i_receiver, receiver in enumerate(scan_data.receivers):
            if (
                receiver_mask_poly_fit[i_receiver] > 0
        ...
        lines = [
            "...........................",
            "Running NoiseDiodePlugin with " + f"MuSEEK version: {branch} ({commit})",
