...
class InpaintingMapmakingPlugin(AbstractParallelJoblibPlugin):
    ...
    def map(
        ...
        crpix_x=int((x_max-x_min)/self.pix_reso/2.)
        crpix_y=int((y_max-y_min)/self.pix_reso/2.)
        ...
        wcs_map.wcs.cdelt = np.array([self.pix_reso, self.pix_reso])  # degrees/pixel for RA and Dec
        wcs_map.wcs.crval = [(x_max+x_min)/2., (y_max+y_min)/2.]
        wcs_map.wcs.ctype = ["RA---ZEA", "DEC--ZEA"]  # Use ZEA projection for RA and Dec
        # Optionally set unit types
        wcs_map.wcs.cunit = ['deg', 'deg']
        
        # set the shape of map
        map_shape = (np.round((x_max-x_min)/self.pix_reso).astype(int) + 1, np.round((y_max-y_min)/self.pix_reso).astype(int) + 1)
    ...
    def run_job(
        ...
        calibrated_data_ant = calibrated_data_ant / 10**6.  ### convert to K  
        ...
        calibrated_data_ant = calibrated_data_ant - np.ma.mean(calibrated_data_ant, axis=0, keepdims=True)
        ...
        delta = (calibrated_data_ant[:,1:-2] + calibrated_data_ant[:,2:-1] -
                 calibrated_data_ant[:,:-3] - calibrated_data_ant[:,3:]) / 2.
        ...
        if calibrated_data_ant.mask.all():
            ...
            for i_time in np.arange(calibrated_data_ant.shape[0]):
                if calibrated_data_ant[i_time,:].mask.all():
                    ...
                    calibrated_data_ant_interp.data[i_time,:] = fill_masked_regions_polyfit(
                        freq_select / 10**6,
                        calibrated_data_ant[i_time,:],
                        polyfit_window=self.inpainting_window,
                        polydeg=self.inpainting_polydeg,
                    )
                    calibrated_data_ant_interp.mask[i_time,:] = np.zeros(len(freq_select),dtype='bool')
        ...
        sky_sc=ac.SkyCoord(ra=ra_ant.flatten() * u.deg, dec=dec_ant.flatten() * u.deg) #pointings in observation
    ...
    def gather_and_set_result(
        ...
        if self.do_store_context:
            ...
            with open(output_path+'/map_making_'+block_name+'_inpainting.pkl', 'wb') as f:
