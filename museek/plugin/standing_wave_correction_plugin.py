...
class StandingWaveCorrectionPlugin(AbstractPlugin):
    ...
    def set_requirements(self):
        ...
        self.requirements = [
            ...
            Requirement(
                location=ResultEnum.STANDING_WAVE_CHANNELS, variable="target_channels"
            ),
            Requirement(
                location=ResultEnum.STANDING_WAVE_EPSILON_FUNCTION_DICT,
                variable="epsilon_function_dict",
            ),
            Requirement(
                location=ResultEnum.STANDING_WAVE_LEGENDRE_FUNCTION_DICT,
                variable="legendre_function_dict",
            ),
            Requirement(
                location=ResultEnum.STANDING_WAVE_CALIBRATOR_LABEL,
                variable="calibrator_label",
            ),
        ]
    ...
    def run(
        ...
        for i_receiver, receiver in enumerate(scan_data.receivers):
            ...
            epsilon = epsilon_function_dict[receiver.name][calibrator_label](
                frequencies
            )
            legendre = legendre_function_dict[receiver.name][calibrator_label](
                frequencies
            )
            self.plot_individual_swings(
                scan_data=scan_data,
    ...
    def plot_individual_swings(
        ...
        colors = jet(
            np.arange(len(swing_turnaround_dumps)) / len(swing_turnaround_dumps)
        )
        ...
        for i in range(len(swing_turnaround_dumps) - 1):
            ...
            flags = scan_data.flags.get(
                time=times, freq=target_channels, recv=receiver_index
            )
            mean_bandpass = scan_data.visibility.get(
                time=times, freq=target_channels, recv=receiver_index
            ).mean(axis=0, flags=flags)
            ...
            fit_frequencies = (
                scan_data.frequencies.get(freq=target_channels).squeeze / MEGA
            )

            corrected = bandpass / model_bandpass  # this should be constant at 1
            corrected = self.correct_linear(
                array=corrected, frequencies=fit_frequencies
            )
    ...
    def plot_azimuth_bins(
        ...
        azimuth_digitized, azimuth_bins = self.azimuth_digitizer(
            azimuth=scan_data.azimuth.get(recv=antenna_index)
        )
        corrected_azimuth_binned_bandpasses = []
        for index in range(len(azimuth_bins)):
            time_dumps = np.where(azimuth_digitized == index)[0]
            visibility = scan_data.visibility.get(
                time=time_dumps, freq=target_channels, recv=i_receiver
            )
            flag = scan_data.flags.get(
                time=time_dumps, freq=target_channels, recv=i_receiver
            )
            corrected_azimuth_binned_bandpasses.append(
                visibility.mean(axis=0, flags=flag).squeeze / (1 + epsilon)
            )
