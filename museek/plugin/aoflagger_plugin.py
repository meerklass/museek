...
class AoflaggerPlugin(AbstractParallelJoblibPlugin):
    ...
    def map(
        ...
        for i_receiver, receiver in enumerate(scan_data.receivers):
            ...
            yield receiver_path, visibility, initial_flag, point_source_flag[
                :, :, i_antenna
            ]
    ...
    def run_job(
        ...
        """
        receiver_path, visibility, initial_flag, point_source_flag_recv = anything
        rfi_flag = get_rfi_mask(
            time_ordered=visibility,
            mask=FlagElement(
                array=(initial_flag + point_source_flag_recv)[:, :, np.newaxis]
            ),
        ...
        rfi_flag = np.where(point_source_flag_recv, initial_flag, rfi_flag)
    ...
    def gather_and_set_result(
        ...
        lines = [
            "...........................",
            "Running AoflaggerPlugin with " + f"MuSEEK version: {branch} ({commit})",
