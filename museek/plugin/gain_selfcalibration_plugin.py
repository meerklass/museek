...
class GainSelfCalibrationPlugin(AbstractParallelJoblibPlugin):
    ...
    def map(
        ...
        assert not np.any(freq_map > 10000), (
            "Frequency values from saved map must not exceed 10000, because they should be in MHz"
        )
    ...
    def run_job(
        ...
        if initial_flag.all():
            ...
            for i_timestamp in np.arange(nd_excess_recv.shape[0]):
                if nd_excess_recv.mask[i_timestamp, :].all():
                    ...
                    with warnings.catch_warnings():
                        ...
                        nd_excess_time_mf = moving_median_masked(
                            nd_excess_time, window_size=self.nd_window_movingmedian
                        )
                        ###  flagging outliers
                        nd_excess_time_residual = nd_excess_time_mf - nd_excess_time
                        nd_excess_time.mask = remove_outliers_zscore_mad(
                            nd_excess_time_residual.data,
                            nd_excess_time_residual.mask,
                            self.nd_zscoreflag_threshold,
                        )
                        ###  interpolating masked regions
                        nd_excess_time = interpolate_1d_masked_array(
                            nd_excess_time, kind="linear"
                        )
                        ###  gaussian smoothing
                        nd_excess_recv[i_timestamp] = scipy.ndimage.gaussian_filter(
                            nd_excess_time, sigma=self.nd_gausm_sigma, mode="nearest"
                        )
            ...
            for i_freq in np.arange(nd_excess_recv.shape[1]):
                if nd_excess_recv.mask[:, i_freq].all():
                    ...
                    nd_excess_freq.mask = remove_outliers_zscore_mad(
                        nd_excess_freq.data,
                        nd_excess_freq.mask,
                        self.nd_zscoreflag_threshold,
                    )
                    nd_excess_freq.mask, p_fit = polynomial_flag_outlier(
                        nd_on_index,
                        nd_excess_freq.data,
                        nd_excess_freq.mask,
                    ...
                    p_poly = np.polyfit(
                        nd_on_index[~nd_excess_freq.mask],
                        nd_excess_freq.data[~nd_excess_freq.mask],
                        deg=self.nd_polyfit_deg,
                    )
                    nd_excess_recv_fit[:, i_freq] = np.polyval(
                        p_poly, np.arange(visibility_recv.shape[0])
                    )

        visibility_recv = visibility_recv / (
            nd_excess_recv_fit / np.median(nd_excess_recv_fit)
        )
        del nd_excess_recv_fit
        gc.collect()

        #############   self calibration   ##############
        temperature_noaver_recv = np.ma.masked_array(
            np.zeros(visibility_recv.shape), mask=initial_flag
        )
        ...
        #####  self calibration   #####
        sky_sc = ac.SkyCoord(
            ra=right_ascension.flatten() * u.deg, dec=declination.flatten() * u.deg
        ...
        for i_freq in np.arange(visibility_recv.shape[1]):
            for i_time in np.arange(visibility_recv.shape[0]):
                ...
                model_rebuild.data[i_time, i_freq] = map_model.data[
                    x_index, y_index, i_freq
                ].copy()
                model_rebuild.mask[i_time, i_freq] = map_model.mask[
                    x_index, y_index, i_freq
                ].copy()

        #####  combine the mask of data and model ######
        mask_combine = (
            visibility_recv.mask
            + model_rebuild.mask
            + (~np.isfinite(visibility_recv.data))
            + (~np.isfinite(model_rebuild.data))
        )
        ...
        for i_freq in np.arange(visibility_recv.shape[1]):
            if visibility_recv[:, i_freq].mask.all():
                gain_selfcali_params[
                    "index_freq_" + str(i_freq) + " " + receiver_name
                ] = np.nan
                baseline_selfcali_params[
                    "index_freq_" + str(i_freq) + " " + receiver_name
                ] = np.nan
                ...
                gain, baseline, c_gain, c_base = (
                    self.fit_gain_noconstant_base_Legendre_auto(
                        timestamps,
                ...
                temperature_noaver_recv.data[:, i_freq] = (
                    visibility_recv.data[:, i_freq] - baseline
                ) / gain

        return (
            temperature_noaver_recv,
            gain_selfcali_params,
            baseline_selfcali_params,
            time_scaled,
            map_model,
    ...
    def gather_and_set_result(
        ...
        temperature_noaver_antennas = np.ma.masked_array(
            temperature_noaver_antennas, mask=mask_antennas
        )
        temperature_noaver_antennas = temperature_noaver_antennas.transpose(1, 2, 0)

        self.set_result(
            result=Result(
                location=ResultEnum.GAIN_SELFCALI_PARAMS,
                result=gain_selfcali_params,
                allow_overwrite=True,
            )
        )
        self.set_result(
            result=Result(
                location=ResultEnum.BASELINE_SELFCALI_PARAMS,
                result=baseline_selfcali_params,
                allow_overwrite=True,
            )
        )
        self.set_result(
            result=Result(
                location=ResultEnum.MAP_SELFCALI,
                result=temperature_noaver_antennas,
                allow_overwrite=True,
            )
        )
        self.set_result(
            result=Result(
                location=ResultEnum.MAP_MODEL, result=map_model, allow_overwrite=True
            )
        )
        self.set_result(
            result=Result(
                location=ResultEnum.TIME_SCALED_SELFCALI_POLYFIT,
                result=time_scaled,
                allow_overwrite=True,
            )
        )
        self.set_result(
            result=Result(
                location=ResultEnum.WCS_MODEL, result=wcs_map, allow_overwrite=True
            )
        )
        ...
        lines = [
            "...........................",
            "Running GainSelfCalibrationPlugin with "
    ...
    def robust_slope_intercept(self, y, x):
        ...
        >>> y = np.array([2, 4, 6, 8, 10]) + np.random.normal(
        ...     0, 1, 5
        ... )  # Noisy linear data
    ...
    def fit_gain_noconstant_base_Legendre_auto(
        self,
        t,
        d,
        m,
        ...
        #   • finiteness
        g0_candidates = [
            g for g in dict.fromkeys(g0_candidates) if np.isfinite(g) and g > 0
        ]
        ...
        candidates = sorted(
            {
                (max(0, default_gain + dg), max(0, default_base + db))
                for dg in range(-deg_range, deg_range + 1, deg_step)
                for db in range(-deg_range, deg_range + 1, deg_step)
            },
            key=lambda x: abs(x[0] - default_gain) + abs(x[1] - default_base),
        )
        ...
        P_gain = {k: legval(tt, [0] * k + [1]) for k in range(1, max_gain + 1)}

        P_base = {j: legval(tt, [0] * j + [1]) for j in range(0, max_base + 1)}
