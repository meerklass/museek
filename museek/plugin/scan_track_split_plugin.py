...
class ScanTrackSplitPlugin(AbstractPlugin):
    ...
    def run(
        ...
        """
        scan_observation_start, scan_observation_end = self._scan_observation_start_end(
            data=data
        )
        ...
        if self.do_delete_unsplit_data:
            ...
            self.set_result(
                result=Result(
                    location=ResultEnum.DATA, result=data, allow_overwrite=True
                )
            )
        ...
        self.set_result(
            result=Result(
                location=ResultEnum.SCAN_OBSERVATION_START,
        ...
        self.set_result(
            result=Result(
                location=ResultEnum.SCAN_OBSERVATION_END,
                result=scan_observation_end,
                allow_overwrite=False,
            )
        )

        # Make flagging report
        ...
        if scan_data is not None and scan_data.flags is not None:
            receivers_list, flag_percent = flag_percent_recv(scan_data)
            lines += ["Scan data flag fraction per receiver:"] + [
                f"{x}  {y}" for x, y in zip(receivers_list, flag_percent)
            ]
        elif not self.keep_scan:
            lines += ["Scan data not kept."]
        if track_data is not None and track_data.flags is not None:
            receivers_list, flag_percent = flag_percent_recv(track_data)
            lines += ["Track data flag fraction per receiver:"] + [
                f"{x}  {y}" for x, y in zip(receivers_list, flag_percent)
            ]
        elif not self.keep_track:
    ...
    def _scan_observation_start_end(data: TimeOrderedData) -> tuple[float, float]:
        ...
        scan_dumps = [
            d
            for t in data._scan_tuple_list
            if t.state == ScanStateEnum.SCAN
            for d in t.dumps
        ]
