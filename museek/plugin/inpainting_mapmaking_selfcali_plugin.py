...
class InpaintingMapmakingSelfcaliPlugin(AbstractParallelJoblibPlugin):
    """Plugin to inapinting and mapmaking, using post self-calibrated data. This plugin is used for
    ...
    def set_requirements(self):
        ...
        self.requirements = [
            ...
            Requirement(
                location=ResultEnum.MAP_SELFCALI, variable="selfcalibrated_data"
            ),
    ...
    def run_job(
        ...
        delta = (
            selfcalibrated_data_ant[:, 1:-2]
            + selfcalibrated_data_ant[:, 2:-1]
            - selfcalibrated_data_ant[:, :-3]
            - selfcalibrated_data_ant[:, 3:]
        ) / 2.0
        ...
        if selfcalibrated_data_ant.mask.all():
            ...
            for i_time in np.arange(selfcalibrated_data_ant.shape[0]):
                if selfcalibrated_data_ant[i_time, :].mask.all():
                    ...
                    selfcalibrated_data_ant_interp.data[i_time, :] = (
                        fill_masked_regions_polyfit(
                            freq_select / 10**6,
                    ...
                    selfcalibrated_data_ant_interp.mask[i_time, :] = np.zeros(
        ...
        misfit = (selfcalibrated_data_ant_interp <= lo) | (
            selfcalibrated_data_ant_interp >= hi
        )
    ...
    def gather_and_set_result(
        ...
        if self.do_store_context:
            ...
            with open(
                output_path + "/map_making_" + block_name + "_selfcali_inpainting.pkl",
                "wb",
            ) as f:
