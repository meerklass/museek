...
from museek.util.tools import git_version_info, find_continuous_mask_regions, project_2d, polynomial_flag_outlier, fill_masked_regions_polyfit
...
class InpaintingMapmakingSelfcaliPlugin(AbstractParallelJoblibPlugin):
    ...
    def set_requirements(self):
        ...
        self.requirements = [
            ...
            Requirement(location=ResultEnum.MAP_SELFCALI, variable="selfcalibrated_data"),
            ...
            Requirement(location=ResultEnum.WCS_MODEL, variable="wcs_map"),
            Requirement(location=ResultEnum.MAP_MODEL, variable="map_model"),
        ]

    def map(
        self,
        scan_data: TimeOrderedData,
        selfcalibrated_data: np.ma.MaskedArray,
        freq_select: np.ndarray,
        output_path: str,
        block_name: str,
        rfi_list: list[tuple[float, float]],
        wcs_map: astropy.wcs.WCS,
        map_model: np.ma.MaskedArray, 
    ) -> Generator[
        tuple[
            astropy.wcs.WCS,
            np.ndarray,
        ...
        """
        Yield a `tuple` of the results path for one antenna, the scanning selfcalibrated data for one antenna and the flag for one antenna.
        :param scan_data: time ordered data containing the scanning part of the observation
        :param selfcalibrated_data: selfcalibrated data containing the scanning part of the observation
        :param freq_select: frequency for selfcalibrated data, in [Hz]
        :param output_path: path to store results
        :param block_name: name of the data block
        :param rfi_list: list of known rfi 
        :param wcs_map: the wcs for map
        :param map_model: the model for map making cude (produced before selfcalibration)
        """
    ...
    def run_job(
        ...
        freq_resolution = np.abs(np.diff(freq_select).mean()) / 10**6.
        assert freq_resolution < 100, "freq_resolution too large, check unit, it must be in MHz unit"
        # Threshold for what counts as a "long" masked region (e.g., 30 MHz)
        threshold_length = self.threshold_MHz / freq_resolution
        for i_time in np.arange(selfcalibrated_data_ant.shape[0]):
            mask_check = selfcalibrated_data_ant.mask[i_time,:].copy()
            ...
            if any(len(region) >= threshold_length for region in continuous_mask_regions):
                ...
                selfcalibrated_data_ant.mask[i_time, :] = True

        # masking the antennas which are heavily masked (more than 60% is masked)
        if np.mean(selfcalibrated_data_ant.mask)>=0.6:
            selfcalibrated_data_ant.mask = True

        selfcalibrated_data_ant_interp = np.ma.ones_like(selfcalibrated_data_ant)
        # Calculate the ABBA T[n] + T[n+1] - T[n-1] - T[n+2] for weight estimation
        delta = (selfcalibrated_data_ant[:,1:-2] + selfcalibrated_data_ant[:,2:-1] -
                 selfcalibrated_data_ant[:,:-3] - selfcalibrated_data_ant[:,3:]) / 2.
        ...
        if selfcalibrated_data_ant.mask.all():
            ...
            for i_time in np.arange(selfcalibrated_data_ant.shape[0]):
                if selfcalibrated_data_ant[i_time,:].mask.all():
                    ...
                    selfcalibrated_data_ant_interp.data[i_time,:] = fill_masked_regions_polyfit(
                        freq_select / 10**6,
                        selfcalibrated_data_ant[i_time,:],
                        polyfit_window=self.inpainting_window,
                        polydeg=self.inpainting_polydeg,
                    )
                    selfcalibrated_data_ant_interp.mask[i_time,:] = np.zeros(len(freq_select),dtype='bool')
                    ...
                    if n_mask > 0:
                        ...
                        selfcalibrated_data_ant_interp.data[i_time, mask] += noise


        # apply the known RFI masking
        selfcalibrated_data_ant_interp.mask[:,mask_freq] = True
        
        # in case there are some misfitting points, mask them
        lo = np.ma.min(selfcalibrated_data_ant) - 0.5
        hi = np.ma.max(selfcalibrated_data_ant) + 0.5
        misfit = (selfcalibrated_data_ant_interp <= lo) | (selfcalibrated_data_ant_interp >= hi)
        misfit_time = np.sum(misfit, axis=1) > 0
        selfcalibrated_data_ant_interp.mask[misfit_time,:] = True
        ...
        sky_sc=ac.SkyCoord(ra=ra_ant.flatten() * u.deg, dec=dec_ant.flatten() * u.deg) 
        pix_coords=skycoord_to_pixel(sky_sc, wcs_map)

        for i_freq, _ in enumerate(freq_select):

            data = selfcalibrated_data_ant_interp[:,i_freq]
            ...
            _, _, hit, _, _ = project_2d(
                pix_coords[1],
                pix_coords[0],
                selfcalibrated_data_ant[:,i_freq].flatten(),
                map_shape,
                weights=weight_diff.flatten()
            )
        ...
        map_making_data_ant = np.ma.masked_array(map_making_data_ant, mask=~np.isfinite(map_making_data_ant))


        del weight_data_ant, selfcalibrated_data_ant, selfcalibrated_data_ant_interp
        gc.collect()

        return map_making_data_ant, hit_data, hit_data_nomask, mask_freq
    ...
    def gather_and_set_result(
        ...
        delta = (map_making_data[:,:,:,1:-2] +
                 map_making_data[:,:,:,2:-1] -
                 map_making_data[:,:,:,:-3] -
                 map_making_data[:,:,:,3:]) / 2.
        weight = 1. / np.ma.std(delta, axis=-1)
        weight_ex = np.repeat(weight[:,:,:,np.newaxis], map_making_data.shape[-1], axis=3)
        ...
        map_antennacombine.mask[mask_antennacombine_sum_freqmedian<=self.mask_antnum_threshold] = True
            ...
            for pixel_j in range(map_antennacombine.shape[1]):
                if map_antennacombine[pixel_i,pixel_j].mask.all():
                    ...
                    with warnings.catch_warnings():
                        ...
                        initial_mask, _ = polynomial_flag_outlier(freq_select, map_antennacombine.data[pixel_i,pixel_j],
        ...
        map_antennacombine = np.ma.masked_array(map_antennacombine.data, mask=mask_polyfit)
        ...
        # Calculate the expression T[n] + T[n+1] - T[n-1] - T[n+2] for valid indices
        delta = (map_antennacombine[:,:,1:-2] +
                 map_antennacombine[:,:,2:-1] -
                 map_antennacombine[:,:,:-3] -
                 map_antennacombine[:,:,3:]) / 2.
            ...
            for pixel_j in range(map_antennacombine.shape[1]):
                if map_antennacombine[pixel_i,pixel_j].mask.all():
                    ...
                    with warnings.catch_warnings():
                        ...
                        mask = map_antennacombine[pixel_i,pixel_j].mask 
                        map_antennacombine_interp.data[pixel_i,pixel_j] = fill_masked_regions_polyfit(                                                                                      
                            freq_select, 
                            map_antennacombine[pixel_i,pixel_j], 
                            polyfit_window=self.inpainting_window, 
                            polydeg=self.inpainting_polydeg,
                        )
                        map_antennacombine_interp.mask[pixel_i,pixel_j] = np.zeros(len(freq_select),dtype='bool') 
                        n_mask = np.count_nonzero(mask)
                        ###  add white noise to masked regions
                        if n_mask > 0:
                            noise = np.random.normal(0.0, delta_sigma_pixel, n_mask)
                            map_antennacombine_interp.data[pixel_i,pixel_j,mask] += noise
        ...
        if self.do_store_context:
            ...
            with open(output_path+'/map_making_'+block_name+'_selfcali_inpainting.pkl', 'wb') as f:
