...
class NoiseDiodeSignalPlugin(AbstractParallelJoblibPlugin):
    ...
    def __init__(self,
        ...
        self.firing_on_dumps = None  # list of on-dump (relative) index arrays, one per firing
        self.firing_periods = None  # period label per firing, aligned with the firing axis
        self.noise_diode_duty = None  # diode on-fraction per dump (duration / dump_period)
    ...
    def map(self,
        ...
        if 'noise_diode_on' not in track_data.flags.flag_names:
            raise ValueError("NoiseDiodeSignalPlugin requires a 'noise_diode_on' flag; "
        ...
        noise_on = np.asarray(track_data.flags.get(freq=0, recv=0).array[nd_i]).squeeze().astype(bool)
        ...
        # ratio of each dump.
        noise_diode = NoiseDiode(dump_period=track_data.dump_period,
        ...
        cycle_starts = noise_diode._get_noise_diode_cycle_start_times(timestamps=track_data.timestamps)
        ratios = noise_diode._get_noise_diode_ratios(timestamps=track_data.timestamps,
                                                     noise_diode_cycle_starts=cycle_starts,
                                                     dump_period=track_data.dump_period)
        timestamps_arr = track_data.timestamps.squeeze
        timestamps_arr = timestamps_arr - timestamps_arr.min()  # epoch seconds -> seconds since start
        noise_on_timestamp = []
        for on_dumps, _off1, _off2, _edge in firings:
            if len(on_dumps) == 1:
                ...
                if weights.sum() > 0:
                    ts = float(np.sum(timestamps_arr[on_dumps] * weights) / np.sum(weights))
    ...
    def run_job(self, anything: tuple[np.ndarray, np.ndarray, list, np.ndarray]) -> np.ndarray:
        ...
        for i_firing, (on_dumps, off1, off2, edge) in enumerate(firings):
            if edge:
                ...
                continue
            on_value = vis[on_dumps].sum(axis=0)          # firing dumps were unflagged -> never masked
            off_value = (vis[off1] + vis[off2]) / 2.0     # masked where either off-neighbour is flagged
    ...
    def gather_and_set_result(self,
        ...
        for period in calibrator_validated_periods:
            ...
            for i_receiver in range(n_receivers):
                freqmedian = np.ma.median(noise_diode_excess[p_sel, :, i_receiver], axis=1)
                firing_outlier[p_sel, i_receiver] = remove_outliers_zscore_mad(
                    freqmedian.data, np.ma.getmaskarray(freqmedian),
                    self.zscoreflag_threshold, self.max_masked_fraction
                )
        full_mask = np.ma.getmaskarray(noise_diode_excess) | firing_outlier[:, np.newaxis, :]
        ...
        for i_receiver, receiver in enumerate(track_data.receivers):
            ...
            lowlim_triggered = bool(np.ma.is_masked(median_excess)
        ...
        flag_array = np.zeros(track_data.visibility.shape, dtype=bool)  # (n_time, n_freq, n_receivers)
        ...
        track_data.flags.add_flag(
            flag=FlagList.from_array(array=flag_array, element_factory=self.data_element_factory),
        ...
        for period in calibrator_validated_periods:
            p_sel = self.firing_periods == period
            excess_by_period[period] = noise_diode_excess[p_sel]                         # (n_firings_p, n_freq, n_recv)
            average_by_period[period] = np.ma.median(noise_diode_excess[p_sel], axis=0)  # (n_freq, n_recv)
        ...
        firings_per_period = {p: int((self.firing_periods == p).sum()) for p in calibrator_validated_periods}
        lines = ['...........................',
                 'Running NoiseDiodeSignalPlugin with ' + f"MuSEEK version: {branch} ({commit})",
        ...
        self.set_result(result=Result(location=ResultEnum.NOISE_DIODE_EXCESS, result=excess_by_period, allow_overwrite=True))
        self.set_result(result=Result(location=ResultEnum.NOISE_DIODE_EXCESS_AVERAGE, result=average_by_period, allow_overwrite=True))
        self.set_result(result=Result(location=ResultEnum.NOISE_DIODE_DUTY_CYCLE, result=self.noise_diode_duty, allow_overwrite=True))
        self.set_result(result=Result(location=ResultEnum.NOISE_ON_TIMESTAMP, result=timestamp_by_period, allow_overwrite=True))
