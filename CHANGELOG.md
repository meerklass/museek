...
* `museek_run_notebook` CLI options renamed for consistency:
  ...
  * `--parameters` short flag `-p` → `-P` to avoid conflict with `--patch`
  * Output directory structure now follows `<base-context-folder>/<patch>/<block-name>/notebook`
* `museek_run_process_uhf_band` CLI option renamed:
...
* Fixed duplication of `context_folder`/`new_output_path` in `AoflaggerPostCalibrationPlugin`
* Stripped unit when passing smoothed map in `AoflaggerPostCalibrationPlugin`
