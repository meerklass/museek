...
class TestFlagList(unittest.TestCase):
    ...
    def test_from_array(self):
        ...
        self.assertEqual(self.flag_list, FlagList.from_array(flag_array, element_factory=DataElementFactory()))
    ...
    def test_from_array_when_3_dimensional(self):
        ...
        self.assertEqual(flag_list, FlagList.from_array(flag_array, element_factory=DataElementFactory()))

    def test_shape(self):
        self.assertTupleEqual((3, 3, 3), self.flag_list.shape)

    def test_add_flag(self):
        self.flag_list.add_flag(flag=DataElement(array=np.zeros((3, 3, 3))))
        flags = [DataElement(array=np.zeros((3, 3, 3))) for _ in range(4)]
        expect = FlagList(flags=flags)
        self.assertEqual(expect, self.flag_list)

    def test_add_flag_when_flag_element(self):
        mock_flags = FlagList(flags=[DataElement(array=np.zeros((3, 3, 3)))])
    ...
    def test_remove_flag(self):
        flag_list = FlagList(flags=[DataElement(array=np.ones((3, 3, 3)) * i) for i in range(3)])
        flag_list.remove_flag(index=1)
        expect = FlagList(flags=[DataElement(array=np.ones((3, 3, 3)) * i) for i in [0, 2]])
        self.assertEqual(expect, flag_list)

    def test_combine_when_empty(self):
        self.assertEqual(DataElement(array=np.zeros((3, 3, 3))), self.flag_list.combine())

    def test_combine_when_ones_and_threshold_small(self):
        flags = [DataElement(array=np.ones((3, 3, 3))) for _ in range(3)]
        flag_list = FlagList(flags=flags)
        self.assertEqual(DataElement(array=np.ones((3, 3, 3))), flag_list.combine(threshold=1))

    def test_combine_when_ones_and_threshold_large(self):
        flags = [DataElement(array=np.ones((3, 3, 3))) for _ in range(3)]
        flag_list = FlagList(flags=flags)
        self.assertEqual(DataElement(array=np.zeros((3, 3, 3))), flag_list.combine(threshold=4))
    ...
    def test_combine_when_different_flags_and_threshold_large(self):
        flags = [DataElement(array=np.ones((3, 3, 3))),
                 ...
                 DataElement(array=np.ones((3, 3, 3)))]
        flag_list = FlagList(flags=flags)
        self.assertEqual(DataElement(array=np.zeros((3, 3, 3))), flag_list.combine(threshold=3))
