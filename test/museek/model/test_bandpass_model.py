...
class TestBandpassModel(unittest.TestCase):
    ...
    def test_fit(
        ...
        frequencies = DataElementFactory().create(
            array=frequency_array[np.newaxis, :, np.newaxis]
        )
        wavelength = 2.0
        mock_estimator_array = (
            300
            + 0.1 * frequency_array
            + 0.1 * np.sin(2 * np.pi / (3e8 / 1e6 / wavelength))
        )
        mock_estimator = DataElementFactory().create(
            array=mock_estimator_array[np.newaxis, :, np.newaxis]
        )
        mock_optimize.curve_fit.return_value = (MagicMock(), MagicMock())
        self.bandpass_model.fit(
        ...
        self.assertEqual(
            mock_parameters_to_dictionary.return_value,
            self.bandpass_model.parameters_dictionary,
        )
        self.assertEqual(
            mock_parameters_to_dictionary.return_value,
            self.bandpass_model.variances_dictionary,
        )
        self.assertEqual(
            mock_bandpass_model.return_value.__truediv__.return_value.__sub__.return_value,
    ...
    def test_bandpass_model(self, mock_sinus, mock_legendre, mock_sum):
        ...
        model = self.bandpass_model._bandpass_model(
        ...
        mock_legendre.legval.assert_called_once_with(
            mock_frequencies, mock_legendre_coefficients
        )
        mock_sinus.assert_called_once_with(
            mock_frequencies, mock_sinus_parameters_list[0]
        )
        self.assertEqual(model, mock_legendre.legval.return_value.__mul__.return_value)

    def test_sinus_expect_zero(self):
        sinus_ = self.bandpass_model._sinus(
            frequencies=np.asarray([0]), parameters=(0, 3, 1)
        )
        self.assertEqual(sinus_[0], 0)

    def test_sinus_expect_1(self):
        sinus_ = self.bandpass_model._sinus(
            frequencies=np.asarray([np.pi / 4 * 1e6]), parameters=(2 * np.pi, 1, 1)
        )
        self.assertAlmostEqual(sinus_[0], 0, 1)

    def test_sinus_parameter_list(self):
        mock_parameters = [1, 2, 3, 4, 5, 6]
        parameter_list = self.bandpass_model._sinus_parameter_list(
            parameters=mock_parameters, n_legendre_coefficients=2
        )
    ...
    def test_parameters_to_dictionary(self):
        mock_parameters = [1, 2, 3, 4, 5, 6]
        dict_ = self.bandpass_model._parameters_to_dictionary(
            parameters=mock_parameters, n_legendre_coefficients=2
        )
        expect = {
            "l_0": 1,
