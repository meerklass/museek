...
class TestClustering(unittest.TestCase):
    ...
    def test_ordered_dumps_of_coherent_clusters(self, mock_split_clusters):
        ...
        ordered_dumps = Clustering().ordered_dumps_of_coherent_clusters(features=mock_features,
    ...
    def test_split_clusters(self):
        ...
        dumps_list, centres = Clustering().split_clusters(feature_vector=mock_timestamps, n_clusters=2)
    ...
    def test_split_pointings(self):
        ...
        up_pointing_centre = np.asarray([centre_pointing_centre[0],
                                         centre_pointing_centre[1] + 0.5])  # 0.5 degree offset
        right_pointing_centre = np.asarray([centre_pointing_centre[0] + 0.7,
                                            centre_pointing_centre[1]])  # 0.7 degree offset
        down_pointing_centre = np.asarray([centre_pointing_centre[0], centre_pointing_centre[1] - 0.5])
        left_pointing_centre = np.asarray([centre_pointing_centre[0] - 0.7, centre_pointing_centre[1]])
        expect_centres = [centre_pointing_centre,
    ...
    def test_ordered_and_unique(self):
        ordered = Clustering._ordered_and_unique(array_or_list=[11, 1, 5, 2, 3, 1, 2, 1, 4, 3, 5, 0])
