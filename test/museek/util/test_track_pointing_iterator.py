...
class TestTrackPointingIterator(unittest.TestCase):
    ...
    def test_iterate_when_two_calibrator_observations(
        self,
        ...
        mock_split_pointings.assert_has_calls(
            [
                call(
                    coordinate_1=track_pointting_iterator._right_ascension.get().squeeze,
                    ...
                    distance_threshold=track_pointting_iterator._distance_threshold,
                )
            ]
            * 2
        )
    ...
    @patch.object(TrackPointingIterator, "_single_dish_calibrators")
    @patch.object(Clustering, "split_pointings")
    def test_iterate_when_one_calibrator_observation(
        self,
        mock_split_pointings,
        mock_single_dish_calibrators,
        mock_two_calibrator_observations,
    ...
    @patch.object(TrackPointingIterator, "_target_dumps_one_calibrator")
    @patch.object(TrackPointingIterator, "_two_calibrator_observations")
    def test_iterate_when_times_none(
        self,
        mock_two_calibrator_observations,
        mock_target_dumps_one_calibrator,
        mock_single_dish_calibrators,
    ...
    def test_target_dumps_two_calibrators_when_else(
        self, mock_ordered_dumps_of_coherent_clusters
    ):
        mock_track_data = MagicMock()
        ...
        target_dumps = track_pointing_iterator._target_dumps_two_calibrators()
        self.assertEqual(
            target_dumps, mock_ordered_dumps_of_coherent_clusters.return_value
        )
    ...
    def test_single_dish_calibrators_when_only_after(self, mock_plt):
        ...
        mock_track_data.timestamps.squeeze = np.asarray(
            [0, 2, 4, 6, 12, 14, 16, 116, 118, 216]
        )
        track_pointing_iterator = TrackPointingIterator(
            track_data=mock_track_data,
            receiver=mock_receiver,
        ...
        result = track_pointing_iterator._single_dish_calibrators(
            target_dumps_list=mock_target_dumps_list, n_calibrator_pointings=2
        )
