...
class TestTimeOrderedDataMapper(unittest.TestCase):
    ...
    def test_grid_when_masked(self):
        ...
        time_ordered_data_mapper = TimeOrderedDataMapper(
            ...
            declination=declination,
            to_map=to_map,
            flags=FlagList.from_array(array=mask_array, element_factory=FlagElementFactory())
        )
        maps, mask = time_ordered_data_mapper.grid(grid_size=(n_dump // 2, n_dump // 2), method='linear')

        self.assertEqual(0, np.nanmin(maps[0]))
        self.assertEqual(0.5, np.nanmax(maps[0]))
        self.assertTupleEqual((n_dump // 2, n_dump // 2), maps[0].shape)
        np.testing.assert_array_equal(np.array([[False, False, False, False, False],
                                                [False, False, False, True, True],
                                                [False, False, False, True, True],
                                                [False, False, False, True, True],
                                                [False, False, False, True, True]]), mask[0])
