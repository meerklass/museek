    ...
    for k in range(data.shape[0]):
        ...
        if ratio < 0.5:  # hard code this?
            ...
            if params.calibration_sources == 'CygA':
                fit, perr, rsquared = fitting.fit_func(x, y, 'gauss2', [params.calibration_chi1, np.mean(deg), 1.0,
                                                                        params.calibration_chi1, np.mean(deg) + 5.0,
                                                                        1.5, 0.0, 1e10])
                fit_results[k, 0] = fit[0]

            else:
                fit, perr, rsquared = fitting.fit_func(x, y, 'gauss',
                                                       [params.calibration_chi1, np.mean(deg), 1.0, 0.0, 1e10])
...
def fit_sun_to_gain(gauss_amp, freq, params):
    ...
    gain_sun = load_file(params.gain_file_sun, skip_header=True, encoding='latin-1')
    sun_freq = gain_sun[:, 0]
    sun_gain = gain_sun[:, 1]
    sun_Ae = gain_sun[:, 3]
    ...
    opt = optimize.minimize(_sun_model_func, (350.0, 0.0001, 1120), args=(cal_freq, cal, sun_gain_interp[mask]),
                            method='Powell')
...
def process_calibration_transits(calibration_paths, ctx):
    ...
    file_path = list(calibration_paths.values())[0][1]
    ...
    for cal_date, cal_paths in calibration_paths.items():
        ...
        sources = np.genfromtxt(cal_paths[0], comments='#', delimiter=',', dtype=None, encoding='latin-1')
